...
import com.apache.minibase.MStore.SeekIter;
...
public class DiskFile implements Closeable {
    ...
    public static class BlockReader {
        ...
        public static BlockReader parserFrom(byte[] buffer, int offset, int size) throws IOException {
            int pos = 0;
            ...
            for (int i = 0; i < kvSize; i++) {
                KeyValue kv = KeyValue.parseForm(buffer, offset + pos);
                kvBuf.add(kv);
    ...
    public static class DiskFileWriter implements Closeable {
        ...
        public void switchNextBlockWriter() throws IOException {
        ...
        public void append(KeyValue kv) throws IOException {
            ...
            if ((currentWriter.getKeyValueCount() > 0) && (kv.getSerializeSize() + currentWriter.size() >= BLOCK_SIZE_UP_LIMIT))
        ...
        public void appendTrailer() throws IOException {
            fileSize = currentOffset + TRAILTER_SIZE;
    ...
    public void open(String filename) throws IOException {
        ...
        assert fileSize > TRAILTER_SIZE;
        in.seek(fileSize - TRAILTER_SIZE);
        ...
        buffer = new byte[8];
        assert in.read(buffer) == buffer.length;
        this.blockIndexOffset = (int) Bytes.toLong(buffer);
    ...
    public BlockReader load(BlockMeta meta) throws IOException{
        in.seek(meta.getBlockOffset());

        byte[] buffer = new byte[(int) meta.getBlockSize()];

        assert in.read(buffer) == buffer.length;
        return BlockReader.parserFrom(buffer, 0, buffer.length);
    }
     ...
     private class InternalIterator implements SeekIter<KeyValue> {
        ...
        public boolean nextBlockReader() throws IOException{
             return false;
        }
